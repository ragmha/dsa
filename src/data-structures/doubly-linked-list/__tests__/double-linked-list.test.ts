...
describe('Doubly Linked List', () => {
  ...
  it('should delete node by value from linked list', () => {
    ...
    expect(linkedList.tail?.previous?.value).toBe(4)
  ...
  it('should be possible to store objects in the list and to print them out', () => {
    const linkedList = new DoublyLinkedList<{ key: string; value: number }>()
    ...
    const nodeStringifier = (value: { key: string; value: number }) =>
  ...
  it('should find node by callback', () => {
    ...
    const node = linkedList.find({
      callback: (value: { key: string }) => value.key === 'test2',
    })
    ...
    expect(
      linkedList.find({
        callback: (value: { key: string }) => value.key === 'test5',
      })
    ).toBeNull()
  })
  ...
  it('should find node by means of custom compare function', () => {
    const comparatorFunction = (
      a: { customValue: string },
      b: { customValue: string }
    ) => {
    ...
    const anotherNode = linkedList.find({
